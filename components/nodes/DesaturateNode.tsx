...
interface DesaturateNodeProps extends NodeProps {
  data: Record<string, never>;
}

const DesaturateNode = ({ id, selected }: DesaturateNodeProps) => {
  return (
    <Card className={`min-w-[200px] ${selected ? "ring-2 ring-primary" : ""}`}>
      <CardContent className="p-4 space-y-3">
        <div className="space-y-2">
          <div className="text-sm font-semibold">Desaturate</div>
